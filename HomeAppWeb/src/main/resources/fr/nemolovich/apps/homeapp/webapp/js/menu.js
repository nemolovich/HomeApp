...
function resizeContent() {
	...
	var html = $('html');
	var winWidth = html.outerWidth(true);
	var winHeight = html.outerHeight(true);
	$('.dialog').each(function () {
		var dialog = $(this);
		var dialogWidth = dialog.outerWidth(true);
		var dialogHeight = dialog.outerHeight(true);
		var marginTop = Math.min((winHeight / 2) - (dialogHeight / 2), 75);
		dialog.css('top', (winHeight / 2) - (dialogHeight / 2) - marginTop);
		dialog.css('left', (winWidth / 2) - (dialogWidth / 2));
	});
}
